...
def kiem_tra_gdxray_png():
    """Kiểm tra ảnh PNG trong thư mục GDXray"""
    print_f("KIEM TRA ANH PNG TRONG GDXray")
    
    if not os.path.exists("GDXray"):
        print_f("LOI: Khong tim thay thu muc GDXray")
        print_f("Vui long tai GDXray va giai nen vao thu muc GDXray/")
        return False, []
    
    # Tìm tất cả ảnh PNG trong GDXray và các thư mục con
    danh_sach_anh = []
    for root, dirs, files in os.walk("GDXray"):
        for file in files:
            if file.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tiff')):
                danh_sach_anh.append(os.path.join(root, file))
    
    if not danh_sach_anh:
        print_f("KHONG TIM THAY ANH TRONG GDXray")
        print_f("Kiem tra lai thu muc GDXray co chua anh khong")
        return False, []
    
    print_f("TIM THAY " + str(len(danh_sach_anh)) + " ANH TRONG GDXray")
...
def tai_anh_xray(duong_dan):
    ...
    try:
        ...
        # Tự động điều chỉnh kích thước nếu ảnh quá lớn
        h, w = anh.shape
        if h > 2000 or w > 2000:
...
def ap_dung_clahe_cai_tien(anh, clip_limit=3.0, grid_size=(16, 16)):
    ...
    # Tạo CLAHE với tham số tối ưu cho ảnh X-ray
    clahe = cv2.createCLAHE(clipLimit=clip_limit, tileGridSize=grid_size)
    anh_clahe = clahe.apply(anh)
    
    # Kết hợp với độ sáng để cải thiện thêm
    alpha = 1.2  # Tăng độ tương phản
    beta = 10    # Tăng độ sáng nhẹ
...
def hien_thi_ket_qua_ro_net(anh_goc, anh_clahe, anh_morphology, anh_ket_hop):
    ...
    # Tạo figure với kích thước lớn hơn để xem rõ
...
def chay_demo_lam_ro_anh():
    ...
    # Kiểm tra và lấy danh sách ảnh từ GDXray
